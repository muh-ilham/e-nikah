...
import { Plus, Pencil, Trash2, Home, Loader2, Search, ChevronLeft, ChevronRight } from "lucide-react";
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
export default function SatuanPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/admin/master-data/satuan?${params.toString()}`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch("/api/admin/master-data/satuan", {
                method,
    ...
    const handleDelete = async (id: string | number) => {
        try {
            const res = await fetch(`/api/admin/master-data/satuan?id=${id}`, { method: "DELETE" });
            if (res.ok) {
                fetchData();
            } else {
                const err = await res.json();
        ...
        <div className="space-y-6 animate-in fade-in duration-700">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500 font-medium italic">Manajemen daftar satuan dan komando wilayah</p>
                </div>
            ...
            <div className="flex gap-4 bg-white p-4 rounded-xl border border-slate-200/60 shadow-sm">
                <div className="flex-1 max-w-sm space-y-1">
                    <Label className="text-[10px] font-bold uppercase text-slate-400">Cari Satuan</Label>
                    ...
                    <Table>
                        <TableHeader className="bg-slate-50/50">
                            <TableRow className="hover:bg-transparent border-b border-slate-100">
                                ...
                                <TableHead className="text-[10px] font-bold uppercase text-slate-400">Nama Satuan / Kesatuan</TableHead>
                        ...
                        <TableBody>
                            {loading ? (
                                ...
                                <TableRow><TableCell colSpan={3} className="h-32 text-center text-slate-400 italic text-xs">Belum ada data satuan</TableCell></TableRow>
                            )}
                        </TableBody>
                    </Table>
                </CardContent>
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle className="text-lg font-bold uppercase">{isEditing ? "Edit Satuan" : "Tambah Satuan"}</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleSubmit} className="space-y-4 py-2">
                        <div className="space-y-2">
                            <Label className="text-xs font-bold">Nama Satuan / Kesatuan</Label>
                            <Input
                                value={formData.nama}
                                onChange={e => setFormData({ ...formData, nama: e.target.value })}
                                placeholder="Contoh: YONIF 201, KOSTRAD, dll"
